...
//标出相机的原点P0
float part1Time = 21.; //P0 00:21  21
//标出相机的look At Q点并连接
float part2Time = 7.; //P0 Q 00:28 7
//标出y轴
float part3Time = 6.; //P0,Q,y 00:34 6
//标出x轴
float part4Time = 15.;//p0,y,x 00:49 15
//标出z轴以及生成相机覆盖线
float part5Time = 24.;//p0,y,x,z  01:13 24
//标出任意点P并虚线连接，
float part6Time = 7.;//P0,P 1:20 7 
//屏幕上画出投影点并打出x'和y'，并且标出步长t
float part7Time = 31.;//P0,P,t,(x',y') 31
...
float map(vec3 pos){
    ...
    float sum1 = part1Time;
    float sum2 = part2Time + sum1;
    float sum3 = part3Time + sum2;
    float sum4 = part4Time + sum3;
    float sum5 = part5Time + sum4;
    float sum6 = part6Time + sum5;
    float sum7 = part7Time + sum6;
    float alpha;
    if(iTime<sum1){
        alpha = iTime/sum1;
        cameraOrigin = sdSphere(pos,cameraOriginPoint,0.2 * alpha);
        
    }else if(iTime< sum2){
        alpha = (iTime - sum1)/(sum2 - sum1);
        float alpha1 = S(0.,0.5,alpha);
        float alpha2 = S(0.5,1.,alpha);
        cameraOrigin = sdSphere(pos,cameraOriginPoint,0.2 );
        cameraLookAt = sdSphere(pos,cameraLookAtPoint,0.2 * alpha1);
        vec2 info = sdSegment(pos,cameraOriginPoint,cameraLookAtPoint);
        originLookAtLine = info.x - 0.04;
        originLookAtLine = smax(originLookAtLine,info.y - alpha2,0.001);
        originLookAtLine = smax(originLookAtLine,abs(sin(info.y * 6. * PI)) - 0.5,0.01);
        
    }else if(iTime< sum3){
        alpha = (iTime - sum2)/(sum3 - sum2);
        cameraOrigin = sdSphere(pos,cameraOriginPoint,0.2 );
        cameraLookAt = sdSphere(pos,cameraLookAtPoint,0.2 );
        vec3 dir = vec3(0.,0.,1.);
        yArrow = sdArrow(pos,cameraOriginPoint,cameraOriginPoint + 2. * dir * alpha,0.05);
        
    }else if(iTime< sum4){
        alpha = (iTime - sum3)/(sum4 - sum3);
        cameraOrigin = sdSphere(pos,cameraOriginPoint,0.2 );
        ...
        xArrow = sdArrow(pos,cameraOriginPoint,cameraOriginPoint + alpha * 2. * xDir ,0.05);
         
    }else if(iTime< sum5){
        alpha = (iTime - sum4)/(sum5 - sum4);
        cameraOrigin = sdSphere(pos,cameraOriginPoint,0.2 );
        float alpha1 = S(0.,0.5,alpha);
        float alpha2 = S(0.5,1.0,alpha);
        vec3 yDir = vec3(0.,0.,1.);
        vec3 xDir = cross(vec3(0.,0.,1.),vec3(0.,1.,0.));
        vec3 zDir = cross(xDir,yDir);
        yArrow = sdArrow(pos,cameraOriginPoint,cameraOriginPoint + 2. * yDir ,0.05);
        xArrow = sdArrow(pos,cameraOriginPoint,cameraOriginPoint + 2. * xDir ,0.05);
        zArrow = sdArrow(pos,cameraOriginPoint,cameraOriginPoint + alpha1 * 2. * zDir ,0.05);
        ...
        frustumEdge = frustumEdgeInfo.x - 0.02;
        frustumEdge = max(frustumEdge,frustumEdgeInfo.y - alpha2);
        frustumEdge = max(frustumEdge,-0.5 + S(-0.5,0.5, sin(100. * frustumEdgeInfo.y)));
        frustumEdge *= 0.3;         
    }else if(iTime< sum6){
        alpha = (iTime - sum5)/(sum6 - sum5);
        cameraOrigin = sdSphere(pos,cameraOriginPoint,0.2 );
        vec3 p = pos;
        p.x = abs(p.x);
        p.y = abs(pos.y - screenY);
        ...
        vec3 dir = vec3(1.7 * screenHalfHeight,screenHalfHeight,screenZ) - start;
        vec2 frustumEdgeInfo = sdSegment(p,start,start + 1.2 * dir);
        frustumEdge = frustumEdgeInfo.x - 0.01;
        ...
        float alpha1 = S(0.,0.5,alpha);
        float alpha2 = S(0.5,1.,alpha);
        randomPoint = sdSphere(pos,randomPointCoord,0.6 * alpha1) * 0.5;
        vec2 d2Info = sdSegment(pos,cameraOriginPoint,randomPointCoord);
        float d2 = d2Info.x - 0.03;
        d2 = max(d2,d2Info.y - alpha2);
...
vec3 renderText(vec2 uv, vec3 ro, vec3 ta){
    ...
    if(iTime<sum1){
        ...
        vec2 camerLookAtProjPoint = projectionPoint(cameraLookAtPoint,ro,ta);
        //导入Q点贴图
        col = mix(col,vec3(1.),textsampler(uv,camerLookAtProjPoint,vec2(0.,1./9.),vec2(1./8.,2./8.)).x);
        ...
        col = mix(col,vec3(1.),textsampler(uv,camerLookAtProjPoint,vec2(0.,1./9.),vec2(1./8.,2./8)).x);
        vec3 yDir = vec3(0.,0.,1);
        vec2 yAxisProjPoint = projectionPoint(cameraOriginPoint + max(0.8,alpha * 2.) * yDir,ro,ta);
        //导入y方向贴图
        ...
        vec3 xDir = cross(vec3(0.,0.,1.),vec3(0.,1.,0.));
        vec3 zDir = cross(xDir,yDir);
        vec2 xAxisProjPoint = projectionPoint(cameraOriginPoint + max(0.8,2. * alpha)* xDir,ro,ta);
        //导入x方向贴图
        ...
        vec2 zAxisProjPoint = projectionPoint(cameraOriginPoint + max(0.8,2. * alpha1)* zDir,ro,ta);
        //导入z方向贴图
...
void main(){
    ...
    for(int i=-(AA - 1); i<AA; i++){
        for(int j= -(AA - 1); j<AA; j++){
            uv = 0.9 * (gl_FragCoord.xy + 0.5 * vec2(float(i),float(j)) - vec2(0.2,0.3) * iResolution.xy)/iResolution.y;
            col += render(uv,ro,ta);
        }
    }
