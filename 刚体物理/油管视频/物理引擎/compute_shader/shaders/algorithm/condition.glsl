...
//-----------------核心判断算子，支持外部定义--------------------
float condition(float idx){
    return ((get_float_by_particle_id_offset(iChannel1,idx, 0.)<5000.))?0.:1.;
}

float compare(float idx1, float idx2)
{
    return condition(idx1) - condition(idx2);          
}
...
float update_height(){
    ...
    if(frame>7.5){
        float start = 0.;
        float end = get_height(iChannel1 ) - 1.;
        if(condition(get_float_by_particle_id_offset(iChannel0,start,0.))>0.){
            return 0.;
        }else if(condition(get_float_by_particle_id_offset(iChannel0,end,0.))<1.){
            return get_height(iChannel1);
        }else{
       
            for(float i = 0.; i<32. ; i++){
                float mid = floor((start + end) * 0.5);
                if(end - start < 1.5) break;
                if(condition(get_float_by_particle_id_offset(iChannel0,mid,0.))<0.5){
                    start = mid;
                }else{
                    end = mid;
                }
...
float update_particle(float particle_id, float offset){
    float frame = get_frame();
    if(frame>7.5) return get_float_by_particle_id_offset(iChannel0,particle_id,0.);
