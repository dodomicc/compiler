...
def keep_obj_move_simplified(
    ...
    keep_obj_alive_update(move_obj,start_move_time,end_move_time,scene,move_obj_updater)
    

def loop_animation(start:float,end:float,single_iteration_time:float, animation:Callable)->List[Group]:
    
    times = int(np.ceil((end - start)/single_iteration_time))
    single_iteration_time = (end - start)/times
    res = []
    for i in range(times):
        start_time = start + i * single_iteration_time
        if(start_time<end - 0.5 * single_iteration_time):
            res.append(animation(start_time,single_iteration_time))
    return res

def loop_animation_fix(groups:List[Group],fix_func:Callable):
    for entry in groups:
        fix_func(entry)
